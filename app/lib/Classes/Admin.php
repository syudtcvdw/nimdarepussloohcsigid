...
class Admin extends Classes
{
    ...
    {
        return Cookie::get(ADMIN_SALT) ? true : Session::get(ADMIN_SALT);
    }
    ...
    {
        return Cookie::get(ADMIN_SALT) ? Cookie::get(ADMIN_SALT) : Session::get(ADMIN_SALT);
    }
            ...
            if ($result->rowCount() > 0) {
                ...
                if (_verify_hash($this->useremail, $this->salt)) {
                    ...
                    if ($rememberMe) // sets a cookie for period of 3 months
                        Cookie::set(ADMIN_SALT, $this->salt, Cookie::EXPIRE_THREE_MONTH);
                    _redirect($redirect);
    ...
    {
        Session::remove(ADMIN_SALT);
        Cookie::remove(ADMIN_SALT);
        _redirect($redirect);
    }
